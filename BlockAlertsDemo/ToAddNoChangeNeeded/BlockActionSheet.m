...
- (void)addButtonWithTitle:(NSString *)title type:(NSString*)type block:(void (^)())block atIndex:(NSInteger)index
{
  if (index >= 0)
  {
...
{
  [self addButtonWithTitle:title type:@"destructiveButton" block:block atIndex:-1];
}

- (void)setCancelButtonWithTitle:(NSString *)title block:(void (^)())block
{
  [self addButtonWithTitle:title type:@"cancelButton" block:block atIndex:-1];
}

- (void)addButtonWithTitle:(NSString *)title block:(void (^)())block 
{
  [self addButtonWithTitle:title type:@"button" block:block atIndex:-1];
}

- (void)setDestructiveButtonWithTitle:(NSString *)title atIndex:(NSInteger)index block:(void (^)())block
{
  [self addButtonWithTitle:title type:@"destructiveButton" block:block atIndex:index];
}

- (void)setCancelButtonWithTitle:(NSString *)title atIndex:(NSInteger)index block:(void (^)())block
{
  [self addButtonWithTitle:title type:@"cancelButton" block:block atIndex:index];
}

- (void)addButtonWithTitle:(NSString *)title atIndex:(NSInteger)index block:(void (^)())block 
{
  [self addButtonWithTitle:title type:@"button" block:block atIndex:index];
}
...
- (void)showInView:(UIView *)view
{
  ...
  for (NSArray *block in _blocks)
  {
    ...
    NSString *buttonType = [block objectAtIndex:2];
    
    UIButton *button = [_buttonFactory buttonWithType:buttonType];
